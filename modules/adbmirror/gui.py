...
from modules.adbmirror.capclient import CapClient
from modules.adbmirror.touchclient import TouchClient
from PIL import Image

class Main():
    def __init__(self, size, orig, path):
        #assert len(sys.argv) == 4
        self.size = list(map(int, size.split("x")))     # list(map(int, sys.argv[1].split("x")))
        orig = list(map(int, orig.split("x")))          # list(map(int, sys.argv[2].split("x")))
